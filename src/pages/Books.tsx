...
import { Plus, Search, Edit, BookOpen, Film } from 'lucide-react';
            ...
            <div key={book.id} className="neu-card bg-card rounded-2xl p-5 group">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  book.availableCopies > 0 ? 'bg-accent text-accent-foreground' : 'bg-destructive/10 text-destructive'
                }`}>
              ...
              {isAdmin && (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => navigate(`/books/edit/${book.serialNo}`)}
